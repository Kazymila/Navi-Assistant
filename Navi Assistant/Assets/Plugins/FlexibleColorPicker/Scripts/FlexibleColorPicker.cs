...
public class FlexibleColorPicker : MonoBehaviour
{
    ...
    */
    ...
    public void UpdateCustomMaterial(Material m, bool alpha)
    {   // Update the custom preview material with the current color
        BufferedColor bc = this.bufferedColor;
        PickerType type = PickerType.Preview;
        if (alpha) type = PickerType.PreviewAlpha;

        m.SetInt(SHADER_MODE, 1);
        Color c1 = PickColor(bc, type, Vector2.zero).color;
        Color c2 = PickColor(bc, type, Vector2.one).color;
        if (!alpha)
        {
